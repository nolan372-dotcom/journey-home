...
<!-- Animal being placed -->
<div class="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-7 max-w-2xl">
    <p class="text-xs font-semibold text-orange-500 uppercase tracking-wider mb-2">Placing</p>
    <p class="text-base font-semibold text-orange-900"><?= esc($animal['name']) ?></p>
    <p class="text-sm text-orange-700 capitalize mt-0.5">
        <?= esc($animal['species']) ?><?= $animal['breed'] ? ' · ' . esc($animal['breed']) : '' ?>
        · <?= esc($animal['age_group']) ?> · <?= esc($animal['size']) ?>
    </p>
    <?php if ($animal['notes']): ?>
        <p class="text-xs text-orange-600 mt-2 leading-relaxed"><?= esc($animal['notes']) ?></p>
    <?php endif ?>
</div>
...
<form method="get" action="/placements/new" class="mb-5 flex gap-2 max-w-2xl">
    ...
    <input type="text" name="q" value="<?= esc($q) ?>" placeholder="Search by name, email or species…"
        class="flex-1 rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400">
...
<?php if ($total === 0 && $q === ''): ?>
    <div class="rounded-lg border border-stone-200 bg-white px-5 py-8 text-center max-w-2xl">
        ...
        <a href="/fosters" class="mt-3 inline-block text-xs font-semibold text-orange-600 hover:text-orange-700">Manage foster homes →</a>
    </div>
<?php elseif ($total === 0): ?>
    <div class="rounded-lg border border-stone-200 bg-white px-5 py-8 text-center max-w-2xl">
        <p class="text-stone-400 text-sm">No foster homes match your search.</p>
        <a href="/placements/new?animal_id=<?= $animal['id'] ?>" class="mt-3 inline-block text-xs font-semibold text-orange-600 hover:text-orange-700">Clear search →</a>
    </div>
<?php else: ?>
...
<div class="space-y-2.5 max-w-2xl">
    ...
    <form action="/placements" method="post">
        ...
        <div class="flex items-center justify-between rounded-lg border border-stone-200 bg-white px-5 py-4 hover:border-orange-300 hover:bg-orange-50 transition-colors group">
            <div>
            ...
            <button type="submit"
                class="ml-4 shrink-0 rounded-lg bg-orange-600 px-4 py-2 text-sm font-semibold text-white hover:bg-orange-700 transition-colors">
                Place here
            </button>
        </div>
    </form>
...
<div class="flex items-center gap-1.5 mt-5 max-w-2xl">
    ...
    <a href="?<?= $baseQuery ?>&page=<?= $p ?>"
       class="rounded-lg border px-3 py-1.5 text-sm font-medium transition-colors <?= $p === $page ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-stone-300 text-stone-600 hover:bg-stone-50' ?>">
        <?= $p ?>
    </a>
    <?php endfor ?>

...
<form action="/animals/<?= $animal['id'] ?>" method="post" enctype="multipart/form-data" class="bg-white rounded-lg border border-stone-200 p-6 space-y-5 max-w-2xl">
    <?= csrf_field() ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div class="sm:col-span-2">
            ...
            <div class="flex items-center gap-4">
                <img id="photo-preview" src="<?= esc($animal['photo_url'] ?? '') ?>" alt=""
                    class="<?= $animal['photo_url'] ? '' : 'hidden' ?> h-16 w-16 rounded-lg object-cover border border-stone-200">
                <label class="cursor-pointer rounded-lg border border-stone-300 bg-white px-4 py-2 text-sm text-stone-700 hover:bg-stone-50 transition-colors">
                    <?= $animal['photo_url'] ? 'Change photo' : 'Choose photo' ?>
                    <input type="file" id="photo-input" name="photo" accept="image/*" class="sr-only">
                </label>
                <span id="photo-filename" class="text-sm text-stone-400"><?= $animal['photo_url'] ? 'Current photo kept if no new file chosen' : 'No file chosen' ?></span>
            </div>
        </div>
        ...
        <div class="sm:col-span-2">
            ...
            <div class="flex flex-wrap gap-x-6 gap-y-2">
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="needs_medical" value="1" <?= !empty($animal['needs_medical']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    Medical case
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="needs_behavior" value="1" <?= !empty($animal['needs_behavior']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    Behavior case
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="needs_fenced_yard" value="1" <?= !empty($animal['needs_fenced_yard']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    Requires fenced yard
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="no_other_dogs" value="1" <?= !empty($animal['no_other_dogs']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    No other dogs
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="no_dogs" value="1" <?= !empty($animal['no_dogs']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    No dogs
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="no_cats" value="1" <?= !empty($animal['no_cats']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    No cats
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="no_other_cats" value="1" <?= !empty($animal['no_other_cats']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    No other cats
                </label>
                <label class="flex items-center gap-2 text-sm text-stone-700 cursor-pointer">
                    <input type="checkbox" name="no_kids" value="1" <?= !empty($animal['no_kids']) ? 'checked' : '' ?> class="rounded border-stone-300 text-orange-600 focus:ring-orange-500">
                    No kids
                </label>
            </div>
        </div>
...
<script>
(function () {
    var tags     = <?= json_encode($animal['custom_needs'] ? json_decode($animal['custom_needs'], true) ?? [] : []) ?>;
